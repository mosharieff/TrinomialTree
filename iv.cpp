...
int main()
{
    ...
    double option_price = 2.33; // Setting option market price in order to compute IV

    double v0 = 0.11, v1 = 0.99, diff = 0, vega = 0, dv = 0.01;

    // Calculate IV
    while(true){
        // Compute option price difference
        diff = PRICE(S, K, r, q, v0, dt, nodes, optype) - option_price;

        // Compute vega
        vega = (PRICE(S, K, r, q, v0+dv, dt, nodes, optype) - PRICE(S, K, r, q, v0-dv, dt, nodes, optype))/(2.0*dv);
        
        // Use an iterative process to compute Implied Volatility
        v1 = v0 - diff / vega;
        if(abs(v1 - v0) < 0.0001){
            break;
        }
    ...
    std::cout << "Simulated Option Price: " << PRICE(S, K, r, q, v1, dt, nodes, optype) << std::endl;
    std::cout << std::endl;
